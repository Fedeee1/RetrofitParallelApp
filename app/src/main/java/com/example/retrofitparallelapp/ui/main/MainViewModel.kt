...
class MainViewModel @Inject constructor(
    ...
    private val listUsersMutableSharedFlow = MutableSharedFlow<ErrorModel>()
    val listUsersSharedFlow: SharedFlow<ErrorModel> = listUsersMutableSharedFlow
    fun getListUsers() {
        viewModelScope.launch(Dispatchers.IO) {
            getUserListUseCase().collect {
                when (it) {
                    is BaseResponse.Error -> {
                        listUsersMutableSharedFlow.emit(it.error)
                    }
